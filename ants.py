    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                ...
                elif type(self.ant) == QueenAnt and self.ant.impostor == False:
                    return
                ...
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if type(self.ant.ant) == QueenAnt and self.ant.ant.impostor == False:
                        return
                    self.ant.ant = None
                else:
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if self.container:
            if self.ant == None and not other.container:
                return True
        return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        count = 0
        while current != hive:
            if current.bees != [] and self.min_range <= count and count <= self.max_range:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    food_cost = 6
    impostor = False
    num_queens = 0 # number of queens
    ant_list = [] #ants with doubled damage
    implemented = True   # Change to True to view in the GUI
    # END Problem 9
    ...
    def action(self, colony):
        ...
        def dub_damage(self):
            ...
            while curr.exit != None:
                curr = curr.exit
                if curr.ant != None:
                    if curr.ant.name == 'Bodyguard' or curr.ant.name == 'Tank':
                        if curr.ant.ant != None:
                            if curr.ant.ant not in self.ant_list:
                                curr.ant.ant.damage *= 2
                                self.ant_list += [curr.ant.ant]

                    if curr.ant not in self.ant_list:
                        curr.ant.damage *= 2
                        self.ant_list += [curr.ant]
...
def apply_effect(effect, bee, duration):
    ...
    def new_action(colony):
        nonlocal duration
        effect(old_action)(colony) if duration > 0 else old_action(colony)
        duration -= 1
